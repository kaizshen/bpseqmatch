...
#12/17/2019 v3 => issue with speed
#Added mismatch checking
from itertools import filterfalse

class seqDict(dict):
    """a dictionary that includes contain functions that finds similar keys"""
    def __simContains__(self, key, mismatch):
...
def main():
    user_input = input('(input_file, bp_length, start_diff, min_matches, #_of_misalignment, output_file): ').split()
    text_fl = user_input[0]
    bp_length = int(user_input[1])
    sp_diff = int(user_input[2])
    min_match = int(user_input[3])
    mis_align = int(user_input[4])
    outp_file = user_input[5]
